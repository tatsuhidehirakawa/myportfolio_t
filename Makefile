...
xx: ## (For Windows!)Destroy all Docker image, container and caches.  
	cd .. && cd myportfolio_k; docker container ls -a; docker system df; docker stop $(docker ps -q); docker system prune --volumes -f; docker container prune -f; docker image prune -a -f; docker builder prune -f; docker images -a -f; docker container ls -a; docker system df
